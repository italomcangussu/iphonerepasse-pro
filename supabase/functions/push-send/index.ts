...
async function buildVapidHeaders(
  ...
  const header = uint8ToUrlB64(new TextEncoder().encode(JSON.stringify({ typ: "JWT", alg: "ES256" })));
  const claims = uint8ToUrlB64(new TextEncoder().encode(JSON.stringify({ aud: audience, exp: expiry, sub: subject })));
...
async function encryptPayload(
  ...
  const senderPublicRaw = new Uint8Array(await crypto.subtle.exportKey("raw", senderKp.publicKey));
  ...
  // ECDH.
  const ikm = new Uint8Array(await crypto.subtle.deriveBits({ name: "ECDH", public: receiverKey }, senderKp.privateKey, 256));
  ...
  padded[0] = 0; padded[1] = 0; // 0-padding
  padded.set(payload, 2);

  const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce }, encKey, padded);

  return { body: new Uint8Array(encrypted), salt, localPublicKey: senderPublicRaw };
}
...
function buildInfo(type: string, receiverKey: Uint8Array, senderKey: Uint8Array): Uint8Array {
  ...
  const info = new Uint8Array(label.length + 1 + 2 + receiverKey.length + 2 + senderKey.length);
  let offset = 0;
...
Deno.serve(async (req: Request) => {
  ...
  const isWorker = workerSecret && reqSecret === workerSecret;
  ...
  if (!body?.notification?.title) return jsonResponse({ error: "notification.title required" }, 400);

  const vapidPrivate = Deno.env.get("VAPID_PRIVATE_KEY");
  const vapidPublic = Deno.env.get("VAPID_PUBLIC_KEY");
  const vapidSubject = Deno.env.get("VAPID_SUBJECT") ?? "mailto:admin@iphonerepasse.com.br";
  ...
  await Promise.all(
    (subs as SubRow[]).map(async (sub) => {
      try {
        const vapidHeaders = await buildVapidHeaders(sub.endpoint, vapidPrivate, vapidPublic, vapidSubject);
        const { body: encBody, salt, localPublicKey } = await encryptPayload(notifJson, sub.p256dh, sub.auth);
        ...
        const res = await fetch(sub.endpoint, {
          ...
          headers: {
            ...
            "Crypto-Key": `dh=${uint8ToUrlB64(localPublicKey)};${vapidHeaders["Authorization"].replace("vapid t=", "p256ecdsa=").split(",k=")[0]}`,
  ...
  if (expiredIds.length) {
    await supabase.from("push_subscriptions")
      .update({ is_active: false, last_error_at: new Date().toISOString(), last_error_message: "Endpoint gone (404/410)" })
      .in("id", expiredIds);
  }
