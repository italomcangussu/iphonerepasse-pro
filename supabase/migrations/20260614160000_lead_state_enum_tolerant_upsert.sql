...
-- Defensive enum normalization in upsert_lead_state (2026-06-14).
--
-- Root cause: the n8n AI workflow (Memory 1/2 LLM) sometimes emits lead_state
-- values outside the canonical enums — interest_type "troca" (canonical
-- "trocar"), desired_condition "novo"/"seminovo" (canonical "Novo"/"Seminovo").
-- Because upsert_lead_state is plpgsql, ANY check-constraint violation raises and
-- aborts the WHOLE upsert, so nothing persists. lead_state then comes back null on
-- every GET, the reconciler loses all prior state each turn (trade-in, desired
-- model, etc.), and the routing flow can never reach inventory/simulation.
--
-- Fix: normalize the four text enum fields up front — map known synonyms to the
-- canonical value and coerce any unknown value to NULL (all four columns are
-- nullable), so a single bad value can never drop the entire persisted state.
-- The n8n side is also fixed deterministically (Code in JavaScript2), but this
-- keeps the table tolerant of future LLM drift.
...
begin
  ...
  -- normalize canonical text enums (tolerant: unknown -> null, never raise).
  -- NOTE: jsonb_set(target, path, NULL) returns NULL for the WHOLE document, so a
  -- CASE that falls through to a SQL NULL would wipe v_state entirely. Wrap each
  -- new value in coalesce(..., 'null'::jsonb) so "unknown" becomes a JSON null
  -- literal (which the INSERT then reads back as SQL NULL) without nuking v_state.
