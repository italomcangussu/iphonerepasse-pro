-- Caixa/cabo do trade-in: boolean -> texto livre.
--
-- A pergunta "Possui caixa e cabo originais?" nao tem resposta necessariamente
-- sim/nao: o cliente responde em linguagem natural ("somente caixa", "so o cabo",
-- "caixa e cabo", "nao tenho mais"). Como tradein_has_box_cable era boolean, o
-- upsert_lead_state fazia (v_state ->> 'tradein_has_box_cable')::boolean, e qualquer
-- resposta que nao casasse com o cast (ex.: "somente caixa") lancava
-- "invalid input syntax for type boolean" e ABORTAVA o upsert do lead_state do turno
-- inteiro. Alem disso o campo ficava null e travava a simulacao (tradeinEvaluationComplete)
-- com re-pergunta em loop.
--
-- Caixa/cabo nao altera a cotacao automatica do trade-in (e informativo para a
-- avaliacao humana). Convertemos a coluna para text para aceitar a resposta literal,
-- e o roteamento deterministico (n8n) deixa de gatear a simulacao nesse campo.

alter table public.lead_state
  alter column tradein_has_box_cable type text
  using (
    case
      when tradein_has_box_cable is true then 'sim'
      when tradein_has_box_cable is false then 'nao'
      else null
    end
  );

-- Recria o upsert: tradein_has_box_cable agora e texto livre (nullif/btrim, sem
-- cast ::boolean) e na colisao usa coalesce-preserve (null preserva o valor anterior).
-- Identico ao 20260620230000 exceto pelas duas linhas desse campo.
