-- Expose the lead's ad origin in search_leads so the n8n agent (which builds its `lead`
-- context object from this RPC's items[0]) can recognize a campaign image and greet the
-- customer about the exact device they clicked. Adds source / source_campaign_title /
-- source_ad_context to the returned columns; everything else is identical to
-- 20260612144500_search_leads_stage_history.sql.
