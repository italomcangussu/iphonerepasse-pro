-- Trade-in "asked" latch no estado do lead.
--
-- A IA deve perguntar logo no inicio do atendimento se o cliente tem um aparelho
-- para dar de entrada/troca (aparelho atual), ANTES de avancar para capacidade/
-- estoque/simulacao. Ate aqui essa pergunta era apenas uma instrucao "soft" no
-- prompt da Bia 1, sem rede deterministica: quando o LLM nao perguntava (caso VD,
-- 2026-06-20), o roteamento avancava para entrada/simulacao e a pergunta nunca era
-- feita. Espelhamos o mecanismo de cash_entry_asked com uma coluna sticky-true.
--
-- has_tradein nao serve para isso: o default e false, entao "nunca perguntei" e
-- "perguntei e o cliente nao tem" sao indistinguiveis. tradein_asked resolve isso.
--
-- Sem essa coluna o upsert_lead_state descartaria silenciosamente o campo (lista
-- de colunas explicita), entao o latch precisa entrar no upsert tambem.

alter table public.lead_state
  add column if not exists tradein_asked boolean not null default false;

-- Recria o upsert incluindo tradein_asked (latch one-way, como cash_entry_asked).
