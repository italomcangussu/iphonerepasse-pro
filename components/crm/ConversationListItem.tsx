    ...
    >
      <span className="flex items-start gap-3">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="mt-1 flex min-w-0 items-center gap-1.5 text-ios-caption text-slate-600 dark:text-slate-300">
            ...
            <span className={`min-w-0 flex-1 truncate ${unreadCount > 0 ? 'font-semibold text-slate-800 dark:text-slate-100' : ''}`}>
              {conversation.lastMessage?.direction === 'outbound' ? 'Você: ' : ''}{previewText}
            </span>
            {unreadCount > 0 && (
              <span
                aria-label={`${unreadCount} mensagens não lidas`}
                className="ml-auto inline-flex min-h-6 min-w-6 shrink-0 items-center justify-center rounded-full bg-brand-600 px-1.5 text-ios-caption font-bold text-white"
              >
                {unreadCount}
              </span>
            )}
