...
`POST` and requires the project's legacy service-role JWT in `Authorization`.
Gateway JWT verification remains enabled to validate the signature; the handler
then requires `role = service_role` and the matching project `ref` claims before
creating a client or reading data. Browser user JWTs cannot call it.
