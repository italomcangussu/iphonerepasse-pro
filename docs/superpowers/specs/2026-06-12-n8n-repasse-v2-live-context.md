...
## Pendente — Bucket 3 (carry-forward deterministico)

Os campos abaixo **nao devem** ir para prompt de LLM (causaria alucinacao). Hoje o `POST Lead_State` (que grava a linha inteira lendo do `Edit Fields5`) os **zera** toda rodada porque vem `null` de `$json` (Memory 2 nao os produz), sobrescrevendo o que os branches de inventario/simulador persistiram no turno anterior. Precisam de **carry-forward do `prev`** (nao prompt):

- **Bucket 3:** `stock_city`, `last_inventory_context` (← `Code Build Inventory Lite`/`Node13` ou `prev`), `stock_item_id`, `simulation_done`, `simulation_count`, `last_simulation_total` (← `Parse Simulator`/`prev`), `pix_data_sent`, `conversation_status_next`, `attendance_owner_next`, `sales_stage_next`.
- O node `Code Refresh Lead State Before Switch2` ja faz parte desse carry-forward, mas roda **depois** do Switch3 (so no branch de estoque). A solucao limpa e o `Code in JavaScript` (→ `POST Lead_State`) ler esses campos do `prev`/lead row em vez do `Edit Fields5` quando vierem null.
