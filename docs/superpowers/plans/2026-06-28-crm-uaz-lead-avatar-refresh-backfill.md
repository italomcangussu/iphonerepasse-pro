...
Test missing/incorrect service-role bearer rejection and tenant-safe resolution of the latest non-group UAZ conversation.
...
Compare the bearer token with `SUPABASE_SERVICE_ROLE_KEY`, query lead/conversation/channel by the requested lead ID, call `syncUazLeadAvatar` with trigger `backfill`, and return sanitized fields only.
