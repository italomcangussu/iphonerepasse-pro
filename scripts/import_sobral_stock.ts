...
type Section = 'NOVOS' | 'SEMINOVOS' | 'APARELHOS_EM_PREPARACAO';
...
const STORE_ID = 'st-091b5910-595d-417c-a32b-57be0feb5a9f';
const IMPORT_TAG = 'IMPORT_SOBRAL_LOTE_001';
const SOURCE_DEFAULT = 'Estoque Sobral Print';

const SECTION_MAPPING: Record<Section, { condition: Condition; status: Status }> = {
  NOVOS: { condition: 'Novo', status: 'Disponível' },
  SEMINOVOS: { condition: 'Seminovo', status: 'Disponível' },
  APARELHOS_EM_PREPARACAO: { condition: 'Seminovo', status: 'Em Preparação' }
};

const MODEL_ALIAS: Array<{ pattern: RegExp; model: string }> = [
  { pattern: /\b17\s*AIR\b/, model: 'iPhone 17 Air' },
  { pattern: /\b16PM\b/, model: 'iPhone 16 Pro Max' },
  { pattern: /\b15PM\b/, model: 'iPhone 15 Pro Max' },
  ...
  { pattern: /\b15G\b/, model: 'iPhone 15' },
  { pattern: /\b13G\b/, model: 'iPhone 13' },
  { pattern: /\b11G\b/, model: 'iPhone 11' }
];

const COLOR_ALIAS: Array<{ token: string; value: string }> = [
  { token: 'SPACE GRAY', value: 'Space Gray' },
  { token: 'SKYBLUE', value: 'SkyBlue' },
  { token: 'DESERT', value: 'Desert' },
  { token: 'GRAFITE', value: 'Grafite' },
  { token: 'NATURAL', value: 'Natural' },
...
const getWarrantyEnd = (descriptionUpper: string): string | null => {
  const match = descriptionUpper.match(/GAR\.?\s*(\d{2})\/(\d{4})/);
...
const parseDevice = (rawDescription: string): { type: DeviceType; model: string; capacity: string; color: string } => {
  ...
  if (descriptionUpper.includes('STARLINK MINI')) {
    return { type: 'Acessório', model: 'StarLink Mini', capacity: '', color: 'Branco' };
  }

  if (descriptionUpper.includes('APPLE WATCH 3')) {
    return {
      type: 'Apple Watch',
  ...
  if (descriptionUpper.includes('IPAD 9')) {
...
const toNormalized = (row: RawRow, index: number): { row?: NormalizedRow; warnings: string[]; error?: string } => {
  ...
  try {
    ...
    const warrantyEnd = getWarrantyEnd(descriptionUpper);
    ...
    if (descriptionUpper.includes('WIFI+3G')) {
      notes.push('WIFI+3G');
      observations.push('wifi+3g');
    }
    if (descriptionUpper.includes('ANATEL')) notes.push('ANATEL');
    notes.push(IMPORT_TAG);

    const origin = descriptionUpper.includes('ANATEL') ? 'ANATEL' : SOURCE_DEFAULT;
    ...
    if (!device.type || !device.model || !sectionConfig.condition || !sectionConfig.status) {
      return { warnings, error: `Linha ${index + 1}: campos obrigatórios não mapeados.` };
    }

    const key = imei
      ? `${STORE_ID}|${imei}`
      : `${STORE_ID}|${row.section}|${device.model}|${device.capacity}|${device.color}|${purchasePrice}|${sellPrice}`;
    const id = `stk_sobral_${stableHash(key)}`;
...
const main = async () => {
  ...
  const sourcePath = path.resolve(cwd, 'data/import/sobral_lote_001.json');
  const reportPath = path.resolve(cwd, 'reports/import_sobral_lote_001.json');
  const rowsPath = path.resolve(cwd, 'reports/import_sobral_lote_001_rows.json');
  ...
  if (isApply) {
    ...
    for (const row of existingRows || []) {
      ...
      const key = existingImei.length === 15
        ...
        : `${STORE_ID}|${row.status === 'Em Preparação' ? 'APARELHOS_EM_PREPARACAO' : row.condition === 'Novo' ? 'NOVOS' : 'SEMINOVOS'}|${row.model || ''}|${row.capacity || ''}|${row.color || ''}|${Number(row.purchase_price)}|${Number(row.sell_price)}`;
      byKey.set(key, row);
    }
