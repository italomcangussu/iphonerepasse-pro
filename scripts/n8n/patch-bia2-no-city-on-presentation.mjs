...
// a partial state throws (drift → run the live guard). DRY=1 previews.
...
if (present.length === 0) {
  console.log(JSON.stringify({ skipped: true, reason: 'already patched (no city in presentation)', workflowId: WORKFLOW_ID, node: NODE_NAME }, null, 2));
  process.exit(0);
}
...
const backupPath = `output/n8n/backups/${WORKFLOW_ID}-before-bia2-no-city-presentation-${new Date().toISOString().replace(/[:.]/g, '-')}.json`;
