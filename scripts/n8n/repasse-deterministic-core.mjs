...
const TRADE_IN_QUESTIONS = {
  tradein_capacity: 'Qual armazenamento?',
  tradein_color: 'Qual a cor do seu aparelho?',
  tradein_scratches: 'Apresenta arranhões?',
  tradein_liquid_contact: 'Aparelho já teve contato com líquido?',
  tradein_side_marks: 'Apresenta marcas de uso na lateral?',
  tradein_parts_swapped: 'Já foi realizada a troca de alguma peça?',
  tradein_has_box_cable: 'Possui caixa e cabo originais?',
  tradein_battery_pct: 'Qual % de bateria?',
  tradein_apple_warranty: 'Está dentro da garantia Apple?',
  tradein_warranty_until: 'Se sim, até quando vai a garantia Apple?',
};
...
export function getMissingTradeInFields(state) {
...
export function deriveTradeInDecision(state) {
...
export function buildAtomicTradeInResponse(state, decision = deriveTradeInDecision(state)) {
...
export function resolveSimulationMode(message, deviceCount) {
...
${hasConsentPrompt.toString()}
${hasQuestionnaire.toString()}
${isAffirmative.toString()}
${deriveTradeInDecision.toString()}
${buildAtomicTradeInResponse.toString()}
${resolveSimulationMode.toString()}
`;

export function buildParseMemoryCoreCode() {
  return `
// REPASSE DETERMINISTIC CORE START
${sharedRuntime()}
const repasseDeterministicState = {
  ...memory,
  last_message_content: repasseLastMessageContent,
  message_buffered: currentMessageRaw,
};
const repasseTradeInDecision = deriveTradeInDecision(repasseDeterministicState);
memory.tradein_questionnaire_status = repasseTradeInDecision.status;
memory.missing_tradein_fields = repasseTradeInDecision.missingFields;
memory.can_simulate_tradein = repasseTradeInDecision.canSimulate;
memory.tradein_evaluation_pending = memory.has_tradein === true && repasseTradeInDecision.canSimulate !== true;
if (repasseTradeInDecision.action) {
  memory.context_ready = false;
  memory.shouldSimulateNow = false;
  memory.shouldSearchInventory = false;
  memory.next_best_action = repasseTradeInDecision.action;
  setMainRoute("shouldUseBia1", repasseTradeInDecision.action);
}
if (Array.isArray(memory.desired_devices) && memory.desired_devices.length > 1) {
  memory.simulation_mode = resolveSimulationMode(currentMessageRaw, memory.desired_devices.length);
}
// REPASSE DETERMINISTIC CORE END
`;
}

export function buildBia1ParserCode() {
  return `// REPASSE DETERMINISTIC BIA1 RESPONSE START
${sharedRuntime()}
